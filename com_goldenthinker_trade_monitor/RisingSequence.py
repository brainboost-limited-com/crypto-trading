...
class RisingSequence(Sequence):
    ...
    def add_tick(self,tick):
        if tick > self.last_tick():
            ...
            ds = DroppingSequence(monitor=self.monitor,first_tick=tick,previous_sequence=self,next_sequence=None)
            self.next(ds)
            print(str(self))
    ...
    def analyze(self):
        ...
        analysis['min_tick_value_timestampt'] = self.ticks[0].get_timestampt()
        analysis['max_tick_value_timestampt'] = self.ticks[len(self.ticks)-1].get_timestampt()
        ...
        analysis['biggest_delta'] = max([d.get_delta() for d in self.ticks])
        analysis['smallest_delta'] = min([d.get_delta() for d in self.ticks])
        analysis['average_delta'] = mean([d.get_delta() for d in self.ticks])
        
        analysis['average_delta_time'] = mean([d.get_delta_time() for d in self.ticks])
        analysis['max_delta_time'] = max([d.get_delta_time() for d in self.ticks]) 
        return analysis
    ...
    def should_i_sell_now(self):
        return StrategyHub.strategy_to_use().should_i_sell_now_rising_sequence(self)