...
def crypto_token_tickers():
    ...
    return {k:v for k,v in tickers_all_currencies.items() if (k) in [c+'USDT' for c in crypto_tokens]}
...
while True:
    ...
    while (Date().time_difference_in_min(start_time,current_time) <= 30):
        tickers_old = tickers
        time.sleep(30)
        tickers = crypto_token_tickers()
    ...
    data = list(zip(list(total_earning_for_symbol_sorted_by_highest_value),list(total_earning_for_symbol_sorted_by_highest_value.values()),list(ups_per_symbol_sorted),list(ups_per_symbol_sorted.values()),list(delta_percentage_by_highest_value),list(delta_percentage_by_highest_value.values())))
    ...
    telegram.send_message(message=f'<pre>{table_header}</pre>')
    ...
    data = list(zip(list(total_earning_for_symbol_sorted_by_lowest_value),list(total_earning_for_symbol_sorted_by_lowest_value.values()),list(downs_per_symbol_sorted),list(downs_per_symbol_sorted.values()),list(delta_percentage_by_lowest_value),list(delta_percentage_by_lowest_value.values())))
    ...
    telegram.send_message(message=f'<pre>{table_header_low}</pre>')    
